...
- **Application access logs** written by the app server itself. These contain the requested path **without query parameters**, status code, response time and user-agent; the logged HTTP Referer field can, however, contain a page URL with coordinates (e.g. `/?lat=..&lng=..`) when you navigate within the site — but not your IP (requests reach the app through the proxy tunnel, so the app only sees the proxy's address). Rotated by size (max ~30 MB retained).
...
- A **bcrypt hash of your password** (we never store the plaintext). If you sign in with Google or GitHub instead, there is no password — we store only the verified email address the provider returns (no provider ID or tokens are kept; see "Sub-processors"). For Facebook sign-in we additionally store a stable, app-scoped **Facebook user ID**, because Facebook does not confirm that the email it returns is verified — binding your login to that ID (rather than matching by email) is what prevents someone else from taking over your account. Email addresses from Facebook are treated as unverified until you confirm them.
...
### When you search by address

- When you type an address (the autocomplete on `/best-operator`, the coverage API, or the embeddable widget), we resolve it against our **own self-hosted database of Polish addresses**, built from public government address registers. The geocoding happens entirely in-house — the address you type is **never sent to any third party**, and it is **not stored** by the application or written to the application's logs.
- Like any web request, the request URL — which contains the typed address — appears in the edge proxy's access logs alongside your IP address for up to ~30 days (see edge access logs above).
- If you subscribe to coverage alerts for an address, the resolved display address is saved as the **name** of your saved location on your account, where you can see and delete it at `/account`.
...
| Full IP (in memory only) | Rate limiting — request counters keyed by IP, held in the rate limiter's in-memory store, never written to disk | Legitimate interest (Art. 6(1)(f)) |
