...
- An **`ss_sid` cookie** — a random, meaningless identifier (no account data, no fingerprint) with a 30-day lifetime. We use it to count distinct visitors and concurrent sessions on our self-hosted dashboard and to tell returning browsers from scraping bots. It is never shared with any third party and is not used to profile you.
...
| `ss_sid` cookie | Distinct-visitor counting + bot detection | Legitimate interest (Art. 6(1)(f)) |
