...
- A **session cookie** (`session=...`) created by Flask. It is a *signed client-side* cookie (the payload is carried in the cookie itself and cryptographically signed; there is **no** server-side session store). It carries no identifying information for anonymous visitors — it only remembers your last clicked location during the visit. Strictly necessary for the app to function, so no consent is required (ePrivacy Art. 5(3) strictly-necessary exemption).
...
- A **bcrypt hash of your password** (we never store the plaintext). If you sign in with Google, GitHub or Facebook instead, there is no password — we store the verified email the provider returns and a link to your provider identity (see "Sub-processors").
...
| Truncated IPs (app audit log) | Rate limiting + abuse prevention | Legitimate interest (Art. 6(1)(f)) |
...
| Pseudonymised map-click events | 30 days (auto-deleted on each server boot) |
...
- **Erase** your data ("right to be forgotten") — `/account` has a "Delete account" action that immediately and irreversibly wipes your active account data: your email, password hash, audit log, saved locations, snapshots, and API keys (cascade delete). Two things outlive the deletion: full IPs in the edge proxy's access logs (rotated out within ~30 days) and truncated IPs in any retained app logs. We do not currently keep disaster-recovery backups; if and when we introduce them, deleted data could survive in a backup for that backup's retention window, and this notice will be updated to state it.
