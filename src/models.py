...
class SubmitLocationEvent(db.Model):
    ...
    - No account link — only a `logged_in` boolean; no raw IP, no full
      User-Agent string.
    - `session_hash` is a salted one-way hash of a per-session token — a
      pseudonym, not reversible to a person.
    - `lat_bucket` / `lng_bucket` are rounded to 2 decimal places (~1km
      grid) and stored ONLY for in-Poland clicks; out-of-PL rows keep
      just the boolean, no coordinate.
    - `browser_class` is the bucketed UA category from observability.
    - 30-day retention (DELETE FROM ... WHERE created_at <
      datetime('now', '-30 days')).

    Disclosed in /privacy. Genuinely pseudonymous — carries no key back
    ...
    lat_bucket = db.Column(db.Float, nullable=True)
    lng_bucket = db.Column(db.Float, nullable=True)
